...
import database
from config import LEDGER_CSV, SCHOOLS_CSV, USERS_CSV, LOW_STOCK_THRESHOLD
from utils import calculate_balance, calculate_books_required
...
def _owner(row: Dict[str, Any]) -> str:
    return str(row.get("modified_by") or row.get("created_by") or "").strip()
...
def build_report(school_id: str = "", date_from: str = "", date_to: str = "",
                 staff: str = "", user: str = "", scope_school_id: str = "") -> Dict[str, Any]:
    """Compute every statistic for the given filters.

    scope_school_id  -- hard security scope (non-admin accounts); it always wins.
    school_id        -- the school picked in the dropdown ('' = All Schools).
    """
    ...
    for row in database.read_csv(LEDGER_CSV):
