...
(function () {
  ...
  function isAdmin() { try { return localStorage.getItem('v_role') === 'super_admin'; } catch (e) { return false; } }
  ...
  function refresh() {
    ...
    return loadData().then(renderAll);
  }
  ...
  function init() {
    ...
    document.addEventListener('v-theme-change', function () { if (isAdmin()) renderChart(latest); });
    // Metrics stay current as records are added, edited or deleted elsewhere.
    document.addEventListener('v-data-changed', refresh);
