...
(function () {
  ...
  function shortWhen(ts) {
    ...
    if (m[1] + '-' + m[2] + '-' + m[3] === todayKey) return m[4] + ':' + m[5];
    return m[3] + '/' + m[2];
  }
  ...
  function dayLabel(day) {
    ...
    if (day === key(d)) return 'Today';
    d.setDate(d.getDate() - 1);
    if (day === key(d)) return 'Yesterday';
    return day;
  }
  ...
  function bubble(m, conv) {
    ...
    if (m.undecryptable) {
      html += '<span class="mg-locked">' + LOCK + ' This message was encrypted for another device and cannot be shown here.</span>';
    } else if (m.body) {
    ...
    var receipt = mine
      ...
      : '';
    var clock = String(m.timestamp || '').match(/(\d{2}):(\d{2})/);
    html += '<span class="meta">' + esc(clock ? clock[0] : m.timestamp) + receipt + '</span></div>';
  ...
  function paintThread(page, items, append) {
    ...
    el('mgMeta').textContent = (conv.type === 'announcement' ? 'Announcement channel' :
      ...
      (isPrivate(conv) ? ' · ' + LOCK + ' End-to-end encrypted' : '');
    ...
    api('/messaging/conversations/' + current + '/messages?limit=' + PAGE)
      .then(function (page) {
        ...
        if (count !== offset || box.getAttribute('data-total') !== String(page.total)) {
          ...
          api('/messaging/conversations/' + current + '/read', 'POST').then(loadList);
        }
  ...
  function readBytes(file) {
    return new Promise(function (resolve, reject) {
      var fr = new FileReader();
      fr.onload = function () {
        resolve({ name: file.name, type: file.type, bytes: fr.result });
      };
      fr.onerror = function () { reject(new Error('Could not read that file.')); };
  ...
  /* Builds the request payload, encrypting it end-to-end for private chats. */
  function buildPayload(conv, body, file) {
    ...
    if (!window.E2EE || !E2EE.supported()) {
      return Promise.reject(new Error('This browser cannot encrypt messages, so sending is blocked.'));
    }
    return Promise.resolve(file ? readBytes(file) : null).then(function (raw) {
      var members = conv.members || [];
      return E2EE.encrypt(members, body, raw).then(function (out) {
        var att = null;
        if (out.attachmentBytes) {
          att = { name: 'message.enc', type: 'application/octet-stream',
                  data: E2EE.b64(out.attachmentBytes) };
        }
        return { body: out.body, attachment: att };
      });
    });
  }
  ...
  // ---- search --------------------------------------------------------------
  /* Search runs in the browser: the server holds only ciphertext for private
     chats, so it cannot search them for us (that is the point of E2EE). */
    ...
    api('/messaging/conversations').then(function (r) {
      ...
      box.innerHTML = hits.map(function (h) {
        ...
        return '<div class="mg-hit" data-id="' + esc(h.conversation_id) + '">' +
          ...
          '<div style="font-size:11px;color:var(--v-text-muted)">' + esc(h.timestamp) + '</div></div>';
      }).join('');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.E2EE && E2EE.supported()) {
      E2EE.ready().catch(function () {
        toast('Encryption keys could not be prepared on this device.', 'error');
      });
    } else {
      toast('This browser does not support end-to-end encryption.', 'error');
    }
