...
class Identity:
    ...
    def school_ids(self) -> Optional[List[str]]:
        ...
        staff       : only the schools assigned to them (their own school plus
    ...
    def visible_usernames(self) -> Optional[List[str]]:
        ...
        super_admin : everyone (admins, staff and users).
        staff       : every 'user' account, plus their own (staff may see all
                      user data, but never other staff or administrators).
...
def reset_user_password(username: str, request: PasswordSet, admin: Identity = Depends(require_staff)):
    ...
    if not admin.is_super_admin:
        ...
        visible = {n.lower() for n in (admin.visible_usernames or [])}
        if username.lower() not in visible:
            raise HTTPException(
                status_code=403,
                detail="You can only change passwords for the user accounts of your own schools.")
